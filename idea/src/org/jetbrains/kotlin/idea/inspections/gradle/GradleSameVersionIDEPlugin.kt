...
class GradleSameVersionIDEPlugin : GradleBaseInspection() {
    ...
    override fun buildErrorString(vararg args: Any): String {
        return "The version of Kotlin used for building via Gradle ('${args[0]}') differs from the one bundled into IDE plugin ('${args[1]}')."
    }
    ...
    private inner class MyVisitor : BaseInspectionVisitor() {
        ...
        override fun visitClosure(closure: GrClosableBlock) {
            ...
            val kotlinPluginVersion = getKotlinPluginVersion(kotlinPluginStatement) ?: return
        ...
        private fun getKotlinPluginVersion(classpathStatement: GrCallExpression): String? {
            ...
            if (grLiteral is GrString && grLiteral.injections.size == 1) {
                ...
                val expression = versionInjection.expression as? GrReferenceExpression ?: return null
        ...
        private fun findClassPathStatements(closure: GrClosableBlock): List<GrCallExpression> {
            val applicationStatements = closure.getChildrenOfType<GrCallExpression>() ?: return emptyList()
